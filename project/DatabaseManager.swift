...
public class DatabaseManager {
 ...
 public func transferMoney(from senderId: Int64, to receiverId: Int64, amount: Int64, completion: @escaping (Error?) -> Void) { //tyt
             ...
             self.getUserBalance(for: receiverId) { receiverBalance, receiverError in
                 ...
                 self.updateBalance(for: receiverId, newBalance: newReceiverBalance) { receiverUpdateError in
                     if let receiverUpdateError = receiverUpdateError {
                         ...
                         self.addNewTransferRecord(sender: Int64(senderId) ?? 0, receiver: Int64(receiverId) ?? 0, amount: amount)
                         completion(nil)
                     }
                 }
             }
 ...
 public func transferMoney(to receiverId: Int64, amount: Int64, completion: @escaping (Error?) -> Void) { //tyt
     getUserBalance(for: receiverId) { receiverBalance, receiverError in
         ...
         self.updateBalance(for: receiverId, newBalance: newReceiverBalance) { receiverUpdateError in
             if let receiverUpdateError = receiverUpdateError {
                 ...
                 self.addNewTransferRecord(receiver: Int64(receiverId), amount: amount) //tyt
                 completion(nil)
             }
         }
     }
        ...
        database.collection(Tables.users.rawValue).getDocuments { snapshot, error in
            ...
            for document in documents {
                if let data = document.data() as? [String: Any],
                   ...
                   let email = data["email"] as? String,
                   let balance = data["balance"] as? Int64 {
                    let user = User(name: name, email: email, profilePictureRef: nil, id: 0, balance: balance) // Set id to 0 for now, it will be updated later
                    users.append(user)
                }
            }
            ...
            for i in 0..<users.count {
                dispatchGroup.enter()
                let user = users[i]
                self.database.collection(Tables.users.rawValue).document(user.email).getDocument { snapshot, error in
...
//    internal func getUser(email: String, completion: @escaping (User?) -> Void) { //new
//        database.collection(Tables.users.rawValue).document(email).getDocument { snapshot, error in
//            if let document = snapshot, document.exists {
//                if let data = document.data(),
//                   let name = data["name"] as? String,
//                   let idString = data["id"] as? String,
