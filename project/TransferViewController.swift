...
class TransferViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        senderLabel.inputView = pickerView1
    }
    ...
    override func viewDidLayoutSubviews() {
        ...
        senderLabel.frame = CGRect(x: 50, y: 220, width: 300, height: 50)
    ...
    @objc private func sendButtonPressed(_ sender: UIButton) {
        ...
        if let selectedReceiver = selectedReceiver, let selectedSender = selectedSender {
            let selectedReceiver = selectedReceiver.email
                .replacingOccurrences(of: ".", with: "_")
                .replacingOccurrences(of: "@", with: "_")
            let selectedSender = selectedSender.email
                .replacingOccurrences(of: ".", with: "_")
                .replacingOccurrences(of: "@", with: "_")
            dbManager.transferMoney(from: selectedSender, to: selectedReceiver, amount: Int64(amount) ?? 0) { [weak self] error in
...
extension TransferViewController: UIPickerViewDelegate {
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        view.endEditing(true)
        selectedSender = Accounts.users[row]
    ...
    @objc private func sendButtonPressed(_ sender: UIButton) {
        ...
        if let selectedReceiver = selectedReceiver, let selectedSender = selectedSender {
            ...
            dbManager.transferMoney(from: selectedSenderId, to: selectedReceiverId, amount: Int64(amount) ?? 0, id: latestTransactionId) { [weak self] error in
                ...
                let alert = UIAlertController(title: "Success!", message: "Transaction successful!", preferredStyle: .alert)
        ...
        self.dismiss(animated: true) {
            ...
            if let keyWindow = UIApplication.shared.keyWindow,
               let rootViewController = keyWindow.rootViewController {
                let transaction = LastTransactionViewController()
